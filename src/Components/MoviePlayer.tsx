  ...
  return (
    <div className="h-screen w-screen bg-black">
      <nav className="fixed w-full p-4 z-10 flex flex-row items-center gap-8 bg-black bg-opacity-70">
        <BiArrowBack
          className="text-white cursor-pointer w-[20px] h-[20px] sm:w-[40px] sm:h-[40px]"
          onClick={() => Navigate("/")}
        />
      ...
      {Movie ? (
        <video src={Movie.video} className="h-full w-full" autoPlay controls />
      ) : (
        <video src={"/"} className="h-full w-full" autoPlay controls />
      )}
    </div>
  );
};
