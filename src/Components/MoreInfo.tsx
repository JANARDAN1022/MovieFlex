  ...
  return (
    ...
    <div className="w-full bg-zinc-900 overflow-hidden h-screen flex justify-center items-center">
        ...
        <div className='w-full relative  text-white flex flex-col max-w-2xl bg-black rounded-md h-max'>
         <video  className='w-full   h-full md:h-auto  object-cover brightness-[50%]' 
         autoPlay
         loop
         muted
         poster={Movie.image}
         src={Movie.video} 
         />
         <span className='bg-black rounded-md py-1 px-4 text-xs top-[40%] md:top-[60%] right-2 absolute'>{formatDuration(Movie.duration)}</span>
       <div onClick={()=>Navigate(`/watch/${Movie.name}`)} className='flex flex-row items-center absolute max-[340px]:bottom-[55%] max-[482px]:bottom-[50%] bottom-[40%] sm:bottom-[35%] left-5  gap-5 '>
