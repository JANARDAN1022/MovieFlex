...
const MovieCard = ({ Movie }: MovieCardProps) => {
  ...
  useEffect(() => {
    ...
    if (isHovered) {
      InfoVidRef.current &&
        InfoVidRef.current.addEventListener("timeupdate", handleTimeUpdate);
    } else {
      InfoVidRef.current &&
        InfoVidRef.current.removeEventListener("timeupdate", handleTimeUpdate);
    }
  ...
  return (
    <div
      ...
      className={` transition-all  ease-in-out opacity-70 hover:opacity-100   bg-black text-white relative   ml-[2px] rounded-md `}
    >
      ...
      <div className="flex flex-col justify-between gap-2 py-4 px-6 w-full items-center">
        <span
          className={`${
            Movie.name.length > 30 ? "text-xs w-full" : "text-sm"
          }  font-bold`}
        >
          {Movie.name}
        </span>
        <div className="flex flex-col justify-center items-center w-full">
          <div className="flex gap-2 items-center">
            <button
              onClick={() => Navigate(`/watch/${Movie.name}`)}
              className="flex gap-2 border border-white rounded-full w-8 h-8 justify-center items-center"
            >
              <BsFillPlayFill size={30} className="ml-1" />
            </button>
            <button
              onClick={() => Navigate(`/Info/${Movie.name}`)}
              className="flex  text-white bg-opacity-30 hover:bg-opacity-100 p-2 rounded-full"
            >
