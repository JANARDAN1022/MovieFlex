  ...
  return Movies ? (
    <div className="h-screen  flex flex-col  gap-2 ">
      ...
      {!searchResults ? (
        ...
        <div
          className={` ${
            searchResults.length > 3 ? "h-max" : "h-screen"
          } w-full  px-10 py-16`}
        >
          ...
          <div
            ...
            className={`${
              SlicedResults?.length === searchResults.length ? "hidden" : ""
            } w-full flex  justify-center items-center`}
          >
            <button className="bg-red-500  border-black border-2 w-max p-2 rounded-md  text-white font-bold opacity-70 hover:opacity-100 transition-all ease-in-out">
              Load More
            </button>
          </div>
        </div>
