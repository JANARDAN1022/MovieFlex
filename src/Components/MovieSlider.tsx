...
const MovieSlider = ({ Movies }: MovieSliderProps) => {
  ...
  const PrevArrow = (props: any) => (
    <button {...props} className="slick-arrow slick-prev">
      <AiOutlineArrowLeft
        className={`text-white cursor-pointer p-1 top-0 left-[-2px] absolute z-[100] w-10 bg-[rgba(0,0,0,0.7)] h-[90%] max-h-full  `}
      />
    </button>
  );
  ...
  const NextArrow = (props: any) => (
    <button {...props} className="slick-arrow slick-next">
      <AiOutlineArrowRight
        className={`text-white cursor-pointer  absolute z-[1] p-1 w-10 bg-[rgba(0,0,0,0.7)] h-[90%] right-[-2px] top-0 `}
      />
    </button>
  );
      ...
      {Movies ? (
        <div className="">
          <p className="text-white flex gap-5  text-md md:text-xl mb-2  lg:text-3xl font-semibold ">
            ...
            <Link
              to={`/Category/${Movies[0].genre}`}
              className={`${
                Path.includes("/Category") ? "hidden" : ""
              } text-sm max-[375px]:text-xs max-[300px]:text-[8px] border bg-white w-max gap-2 opacity-70 hover:opacity-100 flex items-center text-black rounded-md px-2 `}
            >
              Explore {Movies[0].genre} Movies{" "}
              <AiOutlinePlus size={18} className="mt-1" />{" "}
            </Link>
          </p>
          <Slider
